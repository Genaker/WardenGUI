...
class WardenManager:
    ...
    def get_projects(self) -> List[Dict]:
        ...
        result = self._run_cmd([
            "bash", "-c", 
            f"find {self.projects_root} -maxdepth 2 -name '.warden' -type d 2>/dev/null"
        ])
        ...
        for warden_dir in result.stdout.strip().split('\n'):
            ...
            env_result = self._run_cmd(["cat", env_file])
    ...
    def get_env_containers(self, env_name: str) -> List[dict]:
    ...
    def _load_volume_sizes(self, env_name: str) -> Tuple[Dict, str]:
    ...
    def get_cached_volume_sizes(self, env_name: str) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    def get_stop_command(self, project_path: str) -> str:
        ...
        return f"cd {project_path} && {self.WARDEN_PATH} env {stop_cmd}"
    
    def get_start_command(self, project_path: str) -> str:
        """Get the start command string."""
        return f"cd {project_path} && {self.WARDEN_PATH} env up -d"
    ...
    def start_environment(self, env_name: str, project_path: str) -> CommandResult:
        ...
        if "has no container to start" in proc_result.stdout:
            result.errors.append("Containers don't exist - use --down flag to create them")
        
        return result
    
    def get_project_url(self, project: Dict) -> str:
